...
public class MouseClickPos : MonoBehaviour
{
    ...
    [SerializeField] TemporaryTray temporaryTray;
    Ray mouseRay;

    void Update()
    {
        if (gameManager.gameStart && !gameManager.isOpenManual)
        {
            mouseRay = Camera.main.ScreenPointToRay(Input.mousePosition);
            Debug.DrawRay(mouseRay.origin, mouseRay.direction*5, Color.red);
            ...
            if (Input.GetMouseButtonDown(0))
            {
                if (Physics.Raycast(mouseRay, out hit, 15))
                {
                    ...
                    //一時保存オブジェクトの場合、一時保存オブジェクトを持つ
                    else if (hit.collider.gameObject.CompareTag("TemporaryObj"))
                    {
                        temporaryTray.IsLift();
                        if (hit.collider.gameObject.TryGetComponent(out IHitClickRay Ihit))
                        {
                            Ihit.HitClickRay();
                        }
